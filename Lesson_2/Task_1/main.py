...
def get_data():
    ...
    for i in range(1, 4):
        file_read = open(f'info_{i}.txt', 'rb')
        data = (file_read.read()).decode('utf-8')
    ...
    for i in range(0, 3):
        out_data = [l, os_prod_list[i], os_name_list[i], os_code_list[i], os_type_list[i]]
...
def write_to_csv(out_list):
    main_data = get_data()
    with open(out_list, 'w', encoding='utf-8') as file:
